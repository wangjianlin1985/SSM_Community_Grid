...
<head id="Head1">
	...
	"menus": [{
		...
		"menus": [{
			...
			"menuname": "个人信息",
			"icon": "icon-nav",
			"url": "<%=basePath%>user/user.jsp"
		},
		{
			"menuid": "12",
			"menuname": "修改密码",
			"icon": "icon-nav",
			"url": "<%=basePath%>user/usermima.jsp"
		}]
	}
	...
	,
	{
		...
		"menus": [
		{
			...
			"icon": "icon-nav",
			"url": "<%=basePath%>user/<%=jcdaohangslist.get(i).get(0).getJcdaohangNeirong() %>"
		}
		...
		,
		{
			...
			"icon": "icon-nav",
			"url": "<%=basePath%>user/<%=jcdaohangslist.get(i).get(j).getJcdaohangNeirong() %>"
		}
		<%} %>
		]
	}
...
<div id="pageloading" style="position:absolute; top:50%; left:50%; margin:-120px 0px 0px -120px; text-align:center;  border:2px solid #8DB2E3; width:200px; height:40px;  font-size:14px;padding:10px; font-weight:bold; background:#fff; color:#15428B;"> 
    ...
    <div region="north" split="true" border="false" style="overflow: hidden; height: 30px;
        ...
        <span style="float:right; padding-right:20px;" class="head">欢迎 <%=userName %>&nbsp;&nbsp;<a href="<%=basePath%>shouye/tuichu.jsp" id="loginOut">安全退出</a></span>
    ...
    <div id="mainPanle" region="center" style="background: #eee; overflow-y:hidden">
        <div id="tabs" class="easyui-tabs"  fit="true" border="false" >
			<div title="欢迎使用" style="padding:20px;overflow:hidden; color:red; " >
				...
				<h1 style="font-size:24px;">* 登录名：<%=userName %></h1>
			</div>
		</div>
    </div>
